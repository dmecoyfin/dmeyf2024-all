...
envg$EXPENV$semilla_primigenia <- 113149
...
HT_tuning_semillerio <- function( pinputexps, semillerio, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    extra_trees = FALSE,
    # Parte variable
    learning_rate = c( 0.406582, 0.406584 ),
    feature_fraction = c( 0.868062	, 0.868063 ),
    
    leaf_size_log = c( -5.846179, -5.846180),   # deriva en min_data_in_leaf
    coverage_log = c( -7.44395, -7.44396)      # deriva en num_leaves
  )
...
g04_semillerio_competencia2 <- function( pnombrewf )
{
  ...
  ht <- HT_tuning_semillerio(
    ...
    bo_iteraciones = 2  # iteraciones inteligentes, apenas 10
  )
