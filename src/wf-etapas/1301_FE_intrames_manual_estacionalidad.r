...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  library(lubridate)
  mes_reciente <- as.Date("2021-06-01")  # Fecha del mes más reciente
  dataset[, pesos := 0.95^(time_length(interval(as.Date(paste0(foto_mes, "01"), "%Y%m%d"), mes_reciente), "months"))]
