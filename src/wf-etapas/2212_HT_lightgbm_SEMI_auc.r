...
EstimarGanancia_AUC_lightgbm <- function(x) {
    ...
    param_completo <- c(envg$PARAM$lgb_basicos, param_train, x)
...
# Replace EstimarGanancia_lightgbm with EstimarGanancia_AUC_lightgbm
EstimarGanancia_lightgbm <- function(x) {
    ...
    if (file.exists("bayesiana.RDATA")) {
        cat(format(Sys.time(), "%Y%m%d %H%M%S"), "\n", file = "z-Rcanresume.txt", append = TRUE)
        
        # Call the new function instead of the old one
        return(EstimarGanancia_AUC_lightgbm(x))
    }
}
